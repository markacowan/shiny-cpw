  ...
  sidebarLayout(
    sidebarPanel(dateRangeInput("daterange1", "Date Range:",
                 ...
                 sliderInput("slider2", label = ("Resolution for map grid (m)"), min = 10, 
                             max = 500, value = 50),
                 sliderInput("intInput", label = ("Survey interval in days (default=months)"), min = 0, 
                             max = 60, value = 0),
    ...
    checkboxGroupInput("variable", "Species to analyse:",
                       c("Brush Wallaby" = "Brush Wallaby","Cat" = "Cat",
                         "Chuditch" = "Chuditch","Echidna"="Echidna","Euro"="Euro","Fox"="Fox",
                         "Grey Kangaroo"="Grey Kangaroo","Numbat"="Numbat",
                         "Possum"="Possum","Quenda"="Quenda","Rabbit"="Rabbit",
                         "Red-tailed Phascogale"="Red-tailed Phascogale","Tammar"="Tammar",
                         "Woylie"="Woylie"))),
    
    mainPanel(tabsetPanel(type = "tabs",
                          
                          tabPanel("Detection Plots", fluidRow(plotOutput("detection"),br(),br(),
                                                               plotOutput("cusumplot"))),
                          tabPanel("IDW_Interpolation", plotOutput("IDW_Interpolation",height=800)),
                          tabPanel("NN_Interpolation", plotOutput("NN_Interpolation",height=800)),
                          tabPanel("Bubble_Plot", plotOutput("Bubble_Plot",height=600)),
                          tabPanel("Activity", plotOutput("Activity",height=600)),
                          tabPanel("Statistical Distribution", fluidRow(plotOutput("cull_frey", height=400),
...
server <- function(input, output) {
  ...
  five <- subset (four, select=c("SpeciesID","ImageDate","LocationID","LocationName","UTM_E","UTM_N","CommonName","Genus","Species"))
 ...
 ## spec_g<-colnames(locations)###****************************************test to make sure column names are the same
  ...
  output$cull_frey<-renderPlot({
    ...
    if (ncol(matrix3)>=i+5){
      ...
      title(main = paste(species,"(interval:",interval,"days)"), line=2.9,
...
##png(file=paste(species, " cullen and frey graph.png", sep=" "))
descdist(vector, discrete = FALSE)
title(main = paste(species), line=2.9,
      cex.main = 1.2,   font.main= 2, col.main= "black")
##dev.off()
}
})
  ...
  ##########################################################################################################################################################################
    ...
    if(interval>0){ 
      ...
      for (i in 1:iterations2){ #######sets up a loop based on number of unique names
        ...
        oc5 <- dcast(oc4, LocationID~idate,fun.aggregate = sum, 
                     na.rm = TRUE, value.var="count") ########## creates a matrix of locations by month for each species
        ...
        for (i in 1:iterations2){ #######sets up a loop based on number of unique names
          ...
          oc3$count<-0 #####creates variable count an dstores 0 in it
          oc4 <- rbind(oc3, oc1)####combines oc3 and oc1 together into oc4
          oc5 <- dcast(oc4, LocationID~yearmonth,fun.aggregate = sum, 
                       na.rm = TRUE, value.var="count") ########## creates a matrix of locations by month for each species
          oc5[, paste(nameused3)] <- rowSums(oc5[, c(2:ncol(oc5))])
